...
@pytest.fixture
def storage_location(tmpdir):
    return Path(tmpdir) / "maps"


@pytest.fixture
def storage(storage_location):
...
def test_save_and_load_map(storage, map, storage_location):
    # save the map
...
def test_save_map_with_existing_location(storage, map, storage_location):
...
def test_save_map_with_existing_file(storage, map, storage_location):
    # create the directory to store the coverage maps
    os.mkdir(storage_location)
...
def test_apply_storage_policy_keep_all(storage, map, storage_location):
